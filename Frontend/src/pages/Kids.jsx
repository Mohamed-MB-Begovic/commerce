...
export default function Kids() {
 ...
 <div className=" ml-8 ">
     <div className="flex justify-between items-center flex-wrap gap-4 mb-6">
              <h2 className="text-lg font-bold">Kid's Collections</h2>
              <div className="flex gap-3">
              <div className="flex items-center relative -ml-2 -z-10">
...
{isLoading &&<div className='w-[90rem] px-4'> 
  ...
  {!filteredProducts && <h2 className="text-center w-full mx-auto">{`no product found like ${searchedText}`}</h2>}
            ...
            <div data-aos="fade-up" data-aos-delay-200 data-aos-duration="1000" className="grid lg:grid-cols-4 md:grid-cols-2  p-8">
              {/* Sample Products */}
              {filteredProducts?.map((product) => (
                <Link to={`/user/product/${product._id}`} key={product._id}>
              <div className="p-4 bg-green- lg:h-[72vh] lg:w-64">
                    <img
                      src={product.thumbnail}
                      alt="Product"
                      // className="w-full h-[47vh] object-cover mb-4"
