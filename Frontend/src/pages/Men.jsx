...
export default function Men() {
  ...
  useEffect(() => {
   const getProduct = async () => {
      try {
        setIsLoading(true);
        const { data } = await axios.get('/api/products/mens');
        setProducts(data);
 ...
 <div className="">
     <div className="flex justify-between items-center flex-wrap gap-4 px-4 mb-6">
              <h2 className="text-lg font-bold">Men's Collections</h2>
...
>
  ...
  <div 
  className="min-w-4xl flex justify-center flex-wrap  gap-5 "
// className="grid grid-cols-2 px-2 sm:grid-cols-2 gap-3 md:grid-cols-3 lg:grid-cols-4 -z-10" 
  >
  ...
  {/* {!filteredProducts && <h2 className="text-center w-full mx-auto">{`no product found like ${searchedText}`}</h2>} */}
            ...
            // className="grid  lg:grid-cols-4 md:grid-cols-2 gap-4 p-8"
className="grid grid-cols-2 px-2 sm:grid-cols-2 gap-3  md:grid-cols-3 lg:grid-cols-4 lg:gap-10" 

            >
              {/* Sample Products */}
              {filteredProducts?.map((product) => (
                  // <Link to={`/user/product/${product._id}`} key={product._id} 
                  // className="p-4 bg-green- lg:h-[72vh] lg:w-64">
                //   <Link
                //   to={`/user/product/${product._id}`}
                //   key={product._id}
                //   className=" hover:shadow-sm bg-green-4  lg:min-h-84 hover:scale-105 transition duration-300"
                // >
                //     <img
                //       src={product.thumbnail}
                //       alt="Product"
                //       className="h-[20vh] lg:h-72 w-full object-cover mb-4"
                //       loading='lazy'
                //     />
                ...
                <Link to={`/user/product/${product._id}`} key={product._id}>
                <div 
                className="lg:h-[50vh] lg:w-[100%]"
                >  
                   <img
                     src={product.thumbnail}
                     alt="Product"
                     // className="h-72 w-full object-cover mb-4"
                     className="h-[30vh] w-full lg:w-[16rem]  lg:h-[40vh] object-cover mb-4 rounded-sm" 
                   />
                   <h3 className="font-bold truncate">{product.name}</h3>
                   <p className="text-gray-500">${product.price}</p>
                 </div>
